...
const Empleados = () => {
  ...
  // Abrir modal edición y guardar original
  const abrirModalEdicion = (empleado) => {
    setEmpleadoEditado({ ...empleado });
    setEmpleadoOriginal({ ...empleado }); // ← Guardar copia original
    setMostrarModalEdicion(true);
  };

  // Guardar cambios de edición (parcial)
  const guardarEdicion = async () => {
    try {
      ...
      const cambios = {};
      Object.keys(empleadoEditado).forEach(key => {
        if (empleadoEditado[key] !== empleadoOriginal[key]) {
          cambios[key] = empleadoEditado[key];
        }
      });
      ...
      const res = await fetch(`http://localhost:3000/api/actualizarempleado/${empleadoEditado.id_empleado}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      setEmpleadoOriginal({ ...empleadoEditado });
